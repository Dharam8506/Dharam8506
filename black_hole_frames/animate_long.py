...
import sys
W, H, FPS, SEC, XF = 1280, 720, 15, 6, 12  # XF = crossfade frames
D = "black_hole_frames/"
N = FPS * SEC
...
DUR = [20, 166, 264, 89, 139, 160]   # seconds, matched to the voiceover
...
for i in map(int, sys.argv[1:]):
    ...
    p = subprocess.Popen([ff, "-y", "-f", "rawvideo", "-pix_fmt", "rgb24", "-s", f"{W}x{H}", "-r", str(FPS),
        "-i", "-", "-c:v", "libx264", "-pix_fmt", "yuv420p", "-crf", "20", "-preset", "fast",
        f"/tmp/scene{i}.mp4"], stdin=subprocess.PIPE, stderr=subprocess.DEVNULL)
    for f in range(N):
        fr = scene(i, f)
        # gentle breathing motion so long scenes never look frozen
        fade = min(1, f / 15, (N - 1 - f) / 15)
        p.stdin.write(np.clip(fr * fade, 0, 255).astype(np.uint8).tobytes())
    p.stdin.close(); p.wait(); print("scene", i, "done", flush=True)
